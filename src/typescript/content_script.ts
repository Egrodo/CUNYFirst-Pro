...
window.addEventListener(
  ...
  (): void => {
    ...
    chrome.storage.local.get(
      ['schoolId'],
      ({ schoolId: cachedId }: { schoolId: string }) => (schoolId = cachedId ? cachedId : '222'),
    ); // Get the ID from the cache. Default to Baruch.
    chrome.storage.onChanged.addListener(
      changes => (schoolId = changes.schoolId ? changes.schoolId.newValue : schoolId),
    ); // Ensure the schoolId is kept up to date.
    ...
    const getPage = (iframe): string => {
      const titleNode =
        iframe.contentDocument.getElementById('DERIVED_REGFRM1_TITLE1') ||
        iframe.contentDocument.getElementById('DERIVED_REGFRM1_SS_TRANSACT_TITLE');
